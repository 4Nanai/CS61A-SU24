...
def about(subject):
    ...
    def feedback(p):
        paragraph = remove_punctuation(p)
        paragraph = lower(paragraph)
        for sub in subject:
            for para in split(paragraph): 
                if para == sub:
                    return True
        return False
    return feedback
...
def accuracy(typed, source):
    ...
    i, total_words, total_correct = 0, len(typed_words), 0
    ...
    if total_words > len(source_words):
        add_null = total_words - len(source_words)
        source_words = source_words + [""] * add_null
...
def wpm(typed, elapsed):
    ...
    total_char = len(typed)
    if total_char == 0:
        return 0.0
    total_word = total_char / 5
    return total_word * 60 / elapsed
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    check_limit = min(diff, key=abs)
    if check_limit <= limit:
        while item < len(diff):
            if diff[item] == check_limit:
                return word_list[item]
            item += 1
    else:
        return typed_word
...
def furry_fixes(typed, source, limit):
    ...
    if limit < 0:
        return 100
    if typed == '':
        return abs(len(source))
    if source == '':
        return abs(len(typed))
    if typed[0] != source[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    if len(typed) == 1 and typed[0] != source[0]:
        if furry_fixes(source[0] + typed, source, 100) == 0:
            return 1
    ...
    if typed[0] != source[0]: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        best_add = source[0] + typed
        best_substitute = source[0] + typed[1:]
        best_remove = typed[1:]

        return 1 + min(minimum_mewtations(best_add[1:], source[1:], limit - 1), minimum_mewtations(best_substitute[1:], source[1:], limit - 1), minimum_mewtations(best_remove, source, limit - 1))
...
def report_progress(typed, source, user_id, upload):
    ...
    user = {'id': user_id, 'progress': correct/len(source)}
    upload(user)
...
def time_per_word(words, timestamps_per_player):
    ...
    time_slot = len(words)
    i, time = 0, [[]] * (len(timestamps_per_player))
    while i < time_slot:
        j = 0
        while j < len(timestamps_per_player):
            time[j] = time[j] + [(timestamps_per_player[j][i+1] - timestamps_per_player[j][i])]
            j += 1
        i += 1
    return words, time
...
def time_per_word_match(words, timestamps_per_player):
    ...
    word, time = time_per_word(words, timestamps_per_player)

    return match(word, time)
...
def fastest_words(match_object):
    ...
    def find_min(match_obj, p, w):
        if p == len(get_all_times(match_object)) - 1:
            return get_time(match_obj, p, w)
        else:
            return min(get_time(match_obj, p, w), find_min(match_obj, p+1, w))

    for w in word_indices:
        time_index = find_min(match_object, 0, w)
        print("DEBUG:", time_index)
        for p in player_indices:
            if time_index == get_time(match_object, p, w):
                fast_word[p] += [get_word(match_object, w)]
                break
